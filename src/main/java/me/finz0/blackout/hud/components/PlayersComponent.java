...
import me.finz0.blackout.module.gui.Players;
import me.finz0.blackout.util.EntityUtils;
...
public class PlayersComponent extends HudComponent<Players> {
    ...
    public void onTick(TickEvent.ClientTickEvent event){
        ...
        for(EntityPlayer player : Wrapper.getWorld().playerEntities){
            ...
            String dist = Wrapper.getPlayer().getDistance(player) <= 16 ? ""+ChatFormatting.RED+(int)Wrapper.getPlayer().getDistance(player)+" " : "";
            StringBuilder sb = new StringBuilder(dist).append(strength).append(color).append(name).append(" ").append(EntityUtils.INSTANCE.getColoredHP(player));
